...
Amby는 어떤 AI 에이전트든 모델 API, 도구 호출, 네트워크 egress 앞단에 끼워 보안 통제와 규제 감사 증거를 자동 생성하는 AI agent security and compliance data plane이다.
...
- OWASP ASI 위협 항목과 런타임 이벤트의 정확한 매핑
- ASI ↔ NIST AI RMF ↔ EU AI Act ↔ 한국 AI 기본법/금융 가이드라인 ↔ 미국 주별 규제 매핑
...
- 미승인 위험 action 0건
- 승인 지연 시간 측정 가능
- ASI02/03/07 이벤트가 감사 로그와 export에 포함
...
- test result → ASI/NIST/EU/Korea evidence mapping
...
- ASI implemented/planned taxonomy view
