...
| P0 | MCP security and authorization profile | `planned` | Tool firewall, MCP/plugin/skill inventory, env-key-only discovery, AIBOM metadata | No formal MCP auth/security checklist exists. `/v1/*` proxy auth is still post-RC hardening. | Define an MCP profile for OAuth/OIDC boundaries, token audience, no token passthrough, least privilege, egress, and approval-required tools. |
...
| Authorization | Tool firewall and approval-required decisions exist for tool calls. | Add MCP-specific token audience, no token passthrough, OAuth/OIDC, and PKCE checks where applicable. |
